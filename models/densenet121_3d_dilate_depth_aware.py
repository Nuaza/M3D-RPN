...
class RPN(nn.Module):
    ...
    def __init__(self, phase, base, conf):
        ...
        # self.bbox_x_loc = RefConv(self.prop_feats[0].out_channels, self.num_anchors, 3, 1)
        # self.bbox_y_loc = RefConv(self.prop_feats[0].out_channels, self.num_anchors, 3, 1)
        # self.bbox_w_loc = RefConv(self.prop_feats[0].out_channels, self.num_anchors, 3, 1)
        # self.bbox_h_loc = RefConv(self.prop_feats[0].out_channels, self.num_anchors, 3, 1)
        ...
        # self.bbox_rY3d_loc = RefConv(self.prop_feats[0].out_channels, self.num_anchors, 3, 1)
...
def build(conf, phase='train'):
    ...
    if 'pretrained' in conf and conf.pretrained is not None:
        ...
        # prop_feats
        src_weight_key = 'module.{}.0.conv1-5.weight'.format('prop_feats')
        src_bias_key = 'module.{}.0.conv1-5.bias'.format('prop_feats')

        dst_weight_key = 'module.{}.0.group_conv.conv1-5.weight'.format('prop_feats_loc')
        dst_bias_key = 'module.{}.0.group_conv.conv1-5.bias'.format('prop_feats_loc')
